...
// 为了提高性能和可维护性，将ID生成移至组件外部（如果在多个地方使用相同的SVG，可以考虑将其放在更全局的范围）
const safeId = (id: string) => `gemini_icon_${id}`;

// 组件内定义默认的SVG尺寸，以防止宽度和高度为null时的问题
export let width = "100%";
export let height = "100%";

// 使用nanoid生成一个安全的ID
const generatedId = nanoid();
const safeGeneratedId = safeId(generatedId);
</script>
